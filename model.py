...
class LlamaForCausalLMWithNumberLinear(LlamaForCausalLM):
    ...
    def _apply_number_linear(self, token_embeddings, input_ids):
        ...
        # Decode tokens to strings and find number tokens
        token_strings = [
            self.config.tokenizer.decode([token_id]) for token_id in input_ids.flatten()
        ]

        # Create a mask for number tokens
        number_mask = torch.tensor(
            [
                bool(re.match(self.number_token_pattern, token))
