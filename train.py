...
def get_prime_numbers(fourier_dim):
    ...
    prime_numbers = [
        x for x in itertools.islice(gen_primes(), n_prime_numbers)
    ]
...
def update_number_embeddings(model, tokenizer, verbose=False, max_single_number=10_000):
    ...
    for number in range(max_single_number+1):  # Check numbers 0-9
        ...
        if len(tokenized) == 1:  # Single token
            single_token_id_to_number[tokenized[0]] = number
    if verbose:
        print(f"Single-token numbers: {[tokenizer.decode([t]) for t in single_token_id_to_number.keys()]}")
...
def preprocess_function(example, tokenizer, max_length):
    ...
    targets = tokenizer(target_text, max_length=max_length, truncation=True, padding="max_length")
...
def main(args):
    ...
    tokenized_test_dataset = test_dataset.map(lambda x: preprocess_function(x, tokenizer, max_length), batched=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eval_steps", type=int, default=500, help="Steps for evaluation")
    parser.add_argument("--logging_steps", type=int, default=500, help="Steps for logging")
    parser.add_argument("--save_steps", type=int, default=1000, help="Steps for saving model checkpoints")
    parser.add_argument("--save_total_limit", type=int, default=2, help="Total limit for saved checkpoints")
